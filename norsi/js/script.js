$(function () {
  ...
  const formHeight = function () {
    let h = $('.form-block.active').outerHeight();
    $('.form_two').css('height', h);
  };

  if ($('#direction').length) {
    $('#direction').on('change', function () {
      if ($(this).val() !== '') {
        let id = '#' + $(this).val(), classID = '.form__direction:not(' + id + ')';

        $(document).find(classID).css('display', 'none');
        $(document).find(id).css('display', 'block');
      } else {
        $(document).find('.form__direction').css('display', 'none');
      }
    ...
    $(window).on('load', function () {
      if ($(window).width() < 1200) {
        if ($('#step2 .direction__block').length) {
          $('#step1 .step__btn').before($('#step2 .direction__block'));
          $('.form__sub').appendTo('#step2');
        }
        formHeight();

        $('.step__btn .btn').on('click', function () {
          let stepData = $(this).data('step'),
            stepId = '#' + stepData;
          ...
          $('.step').each(function () {
            if ($(this).data('step') === stepData) {
              $(this).addClass('active');
            }
          });
          $('.step.active').prev().addClass('done');
        });
      }
    });
  }
  ...
  $('.config__arrow').on('click', function (e) {
    ...
    const tabContent = '.config__content[data-config-content="' + tab + '"]', tabNow = '.config__block[data-config="' + tab + '"]';
    if (!$('.page-order').find(tabContent).hasClass('active')) {
      $('.config__content').removeClass('active');
      $('.page-order').find(tabContent).addClass('active');
      $('.config__block').removeClass('active');
